...
df_payment_two = df.filter((F.col("ContractType") == "Two year"))
df_payment_one = df.filter((F.col("ContractType") == "One year"))
df_payment_month = df.filter((F.col("ContractType") == "Month-to-month"))
...
for df_combination in [df_payment_two, df_payment_one, df_payment_month]:
    ...
    non_churned_risk_df = final_df.filter(F.col("churn") == 0) \
        .select("customerID", "ContractType", "MonthsSubscribed", "Risk Label")
...
final_combined_df.write.parquet("hdfs:///data/processed/contractKMean.parquet")
