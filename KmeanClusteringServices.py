...
df_services_1 = df.filter((F.col("AmountServices") == 1))
df_services_2 = df.filter((F.col("AmountServices") == 2))
df_services_3 = df.filter((F.col("AmountServices") == 3))
df_services_4 = df.filter((F.col("AmountServices") == 4))
df_services_5 = df.filter((F.col("AmountServices") == 5))
df_services_6 = df.filter((F.col("AmountServices") == 6))
df_services_7 = df.filter((F.col("AmountServices") == 7))
df_services_8 = df.filter((F.col("AmountServices") == 8))
df_services_9 = df.filter((F.col("AmountServices") == 9))
...
for df_combination in [df_services_1, df_services_2, df_services_3, df_services_4, df_services_5, df_services_6, df_services_7, df_services_8, df_services_9]:
    ...
    non_churned_risk_df = final_df.filter(F.col("churn") == 0) \
        .select("customerID", "AmountServices", "MonthsSubscribed", "Risk Label")
...
final_combined_df.write.parquet("hdfs:///data/processed/servicesKMean.parquet")
