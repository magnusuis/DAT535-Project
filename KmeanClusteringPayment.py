...
df_payment_electronic = df.filter((F.col("PaymentMethod") == "Electronic check"))
df_payment_mail = df.filter((F.col("PaymentMethod") == "Mailed check"))
df_payment_bank = df.filter((F.col("PaymentMethod") == "Bank transfer (automatic)"))
df_payment_card = df.filter((F.col("PaymentMethod") == "Credit card (automatic)"))
...
for df_combination in [df_payment_electronic, df_payment_mail, df_payment_bank, df_payment_card]:
    ...
    non_churned_risk_df = final_df.filter(F.col("churn") == 0) \
        .select("customerID", "PaymentMethod", "MonthsSubscribed", "Risk Label")
...
final_combined_df.write.parquet("hdfs:///data/processed/paymentKMean.parquet")
